...
it("auto-persists the latest director scene snapshot after scene changes", () => {
  ...
  const snapshot = localStorage.getItem("monto-3d-director-desk-demo");
...
it("keeps persisted director scenes isolated per canvas card instance", () => {
  ...
  expect(localStorage.getItem("monto-3d-director-desk-demo:node_director_a")).not.toBeNull();
  ...
  expect(localStorage.getItem("monto-3d-director-desk-demo:node_director_b")).not.toBeNull();
