...
it("shows a visible viewport message instead of silently blacking out when panorama loading fails", async () => {
  ...
  expect(await screen.findByText("请重新导入 JPG / PNG / WEBP 图片")).toBeInTheDocument();
  expect(mockScene.background).toBeInstanceOf(Color);
});

it("explains CORS when a remote panorama URL fails to load into WebGL", async () => {
  const remoteAsset: DirectorAssetRef = {
    ...panoramaAsset,
    url: "https://cdn.example.com/panorama.webp",
  };

  render(
    <ViewportBackground
      backgroundColor="#123456"
      panoramaAsset={remoteAsset}
      panoramaRadius={60}
      panoramaYaw={0}
    />
  );
  ...
  expect(await screen.findByText("全景图无法贴到场景")).toBeInTheDocument();
  expect(await screen.findByText(/未开放跨域（CORS）/)).toBeInTheDocument();
  expect(await screen.findByText(/侧栏缩略图可以显示/)).toBeInTheDocument();
});
