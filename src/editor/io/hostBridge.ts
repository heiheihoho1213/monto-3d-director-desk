...
export interface DirectorDeskHostCapture {
  dataUrl: string;
  fileName: string;
}
...
export interface DirectorDeskHostConfig {
  ...
  onCapturesSent?: ((captures: DirectorDeskHostCapture[]) => void) | null;
}
...
let enablePostMessage = true;
let applyThemeToDocument = true;
let onCapturesSent: ((captures: DirectorDeskHostCapture[]) => void) | null = null;
...
function notifyPanoramaRemoved() {
  ...
  if (enablePostMessage) {
    window.parent?.postMessage(
      {
        type: "monto:director-desk-panorama-removed",
...
export function configureDirectorDeskHost(config: DirectorDeskHostConfig) {
  if (typeof config.enablePostMessage === "boolean") {
    enablePostMessage = config.enablePostMessage;
  }
  if (typeof config.applyThemeToDocument === "boolean") {
    applyThemeToDocument = config.applyThemeToDocument;
  }
  if ("onCapturesSent" in config) {
    onCapturesSent = config.onCapturesSent ?? null;
  }
}
...
export function postDirectorDeskCapturesToHost(
  ...
  const normalizedCaptures = captures
    ...
    .filter((capture): capture is DirectorDeskHostCapture => Boolean(capture));

  if (normalizedCaptures.length === 0) {
    return;
  }

  onCapturesSent?.(normalizedCaptures);
  ...
  window.parent?.postMessage(
    {
      type: "monto:director-desk-captures-sent",
      payload: {
...
function handleHostMessage(event: MessageEvent) {
  ...
  if (event.data?.type === "monto:director-desk-session") {
    openHostSession((event.data.payload || {}) as HostSessionPayload);
    return;
  }

  if (event.data?.type === "monto:director-desk-panorama") {
